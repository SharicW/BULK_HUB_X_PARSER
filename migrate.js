...
async function migrate() {
  ...
  // 3. Recreate ingest_state with correct schema if it has the old key/value structure
  try {
    const colCheck = await q(`
      SELECT column_name FROM information_schema.columns
      WHERE table_name = 'ingest_state' AND column_name = 'community_id'
    `);
    if (colCheck.rows.length === 0) {
      console.log("ingest_state has old schema, recreating...");
      await q(`DROP TABLE IF EXISTS ingest_state`);
      await q(`
      ...
      `);
      console.log("✓ Recreated ingest_state with correct schema");
    } else {
      console.log("✓ ingest_state schema is correct");
    }
  } catch (e) {
    console.error("Error fixing ingest_state:", e.message);
    throw e;
  }
